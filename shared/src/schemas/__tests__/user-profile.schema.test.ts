...
describe("CreateUserProfileSchema", () => {
  ...
  it("should reject invalid goal", () => {
    expect(() =>
      CreateUserProfileSchema.parse({ ...valid, goal: "bulk" }),
    ).toThrow();
  });

  it("should reject non-positive height", () => {
    expect(() =>
      CreateUserProfileSchema.parse({ ...valid, height: 0 }),
    ).toThrow();
    expect(() =>
      CreateUserProfileSchema.parse({ ...valid, height: -10 }),
    ).toThrow();
  });

  it("should reject non-positive weight", () => {
    expect(() =>
      CreateUserProfileSchema.parse({ ...valid, weight: 0 }),
    ).toThrow();
    expect(() =>
      CreateUserProfileSchema.parse({ ...valid, weight: -10 }),
    ).toThrow();
  });

  it("should reject non-integer age", () => {
    expect(() =>
      CreateUserProfileSchema.parse({ ...valid, age: 25.5 }),
    ).toThrow();
  });
  ...
  it("should reject activity below 1.2", () => {
    expect(() =>
      CreateUserProfileSchema.parse({ ...valid, activity: 1.0 }),
    ).toThrow();
  });

  it("should reject activity above 2.5", () => {
    expect(() =>
      CreateUserProfileSchema.parse({ ...valid, activity: 3.0 }),
    ).toThrow();
  });

  it("should accept activity at boundaries", () => {
    expect(
      CreateUserProfileSchema.parse({ ...valid, activity: 1.2 }).activity,
    ).toBe(1.2);
    expect(
      CreateUserProfileSchema.parse({ ...valid, activity: 2.5 }).activity,
    ).toBe(2.5);
  });
});
