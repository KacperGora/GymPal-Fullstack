...
import { exerciseCategoryEnum, createWorkoutExerciseSchema, updateWorkoutExerciseSchema, createWorkoutSessionSchema, updateWorkoutSessionSchema, workoutQuerySchema, } from "../workouts.schema.js";
...
describe("createWorkoutExerciseSchema", () => {
    ...
    it("should reject non-positive sets", () => {
        expect(() => createWorkoutExerciseSchema.parse({ ...valid, sets: 0 })).toThrow();
    });
    it("should reject non-positive reps", () => {
        expect(() => createWorkoutExerciseSchema.parse({ ...valid, reps: -1 })).toThrow();
    });
    it("should reject negative weight", () => {
        expect(() => createWorkoutExerciseSchema.parse({ ...valid, weight: -5 })).toThrow();
    });
    it("should accept zero weight (bodyweight)", () => {
        expect(createWorkoutExerciseSchema.parse({ ...valid, weight: 0 }).weight).toBe(0);
    });
    it("should reject negative restTime", () => {
        expect(() => createWorkoutExerciseSchema.parse({ ...valid, restTime: -10 })).toThrow();
    });
    it("should accept zero restTime", () => {
        expect(createWorkoutExerciseSchema.parse({ ...valid, restTime: 0 }).restTime).toBe(0);
    });
    it("should reject empty exerciseName", () => {
        expect(() => createWorkoutExerciseSchema.parse({ ...valid, exerciseName: "" })).toThrow();
    });
});
...
describe("createWorkoutSessionSchema", () => {
    ...
    it("should reject non-positive duration", () => {
        expect(() => createWorkoutSessionSchema.parse({ ...valid, duration: 0 })).toThrow();
    });
    it("should reject negative caloriesBurned", () => {
        expect(() => createWorkoutSessionSchema.parse({ ...valid, caloriesBurned: -10 })).toThrow();
    });
    it("should reject empty name", () => {
        expect(() => createWorkoutSessionSchema.parse({ ...valid, name: "" })).toThrow();
    });
});
...
describe("workoutQuerySchema", () => {
    ...
    it("should reject invalid datetime format", () => {
        expect(() => workoutQuerySchema.parse({ startDate: "not-a-date" })).toThrow();
    });
});
