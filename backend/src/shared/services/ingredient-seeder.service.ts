...
interface IngredientSpec {
  /** Polish name stored in DB — used for lookup by AI-generated ingredient strings */
  polishName: string;
  /** English query sent to USDA search API */
  usdaQuery: string;
  category: IngredientCategory;
  /** USDA data types to prefer; defaults to Foundation + SR Legacy */
  dataTypes?: string[];
}

/**
 * Canonical list of ingredients to seed.
 * Polish names match what the AI generates; USDA queries find accurate data.
 * All entries use servingSize=100, servingUnit=g (or ml for liquids) — simplifies math.
 */
const INGREDIENTS_TO_SEED: IngredientSpec[] = [
  // ── Mięso i drób ─────────────────────────────────────────────────────────
  {
  ...
  async seedFromUsda(): Promise<SeedResult> {
    ...
    for (const spec of INGREDIENTS_TO_SEED) {
      try {
        ...
        // Respect USDA rate limit (3500 req/hour ≈ ~1 req/s)
